  ...
  try {
    ...
    if (findUser) {
      ...
      await res.status(404).send({
        status: "error",
        message:
          "User with that email does not exist. Please make sure the email is correct.",
      });
    }
  } catch (err) {
    console.error(err);
...
const sendEmailVerification = async (req, res) => {
  ...
  try {
    ...
    if (findUser) {
      ...
      await sendVerifyEmail(email, "Verify your email", link);
...
const setPassword = async (req, res) => {
  ...
  try {
    ...
    const storedToken = await redis.get(`${email}-setPasswordToken`);
    ...
    await redis.del(`${email}-setPasswordToken`);
...
const setPasswordPage = (req, res) => {
  ...
  if (!token || !email) {
    return res.status(400).send("Invalid or missing token/email.");
  }

  // Render หน้า EJS และส่ง token กับ email ไปยังไฟล์ EJS
  res.render("setPassword", { token, email });
};
...
const checkAccount = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      status: "error",
      message: err.message || "Error occurred while checking the email.",
    });
  }
};
