        ...
        .then(async (existingUser) => {
          if (existingUser) {
            if (!existingUser.user.password) {
              console.log("Go to set password");

              // ส่งอีเมลให้ผู้ใช้ตั้งรหัสผ่านใหม่
              const resetToken = crypto.randomBytes(32).toString("hex");
              redis.set(`${email}-setPasswordToken`, resetToken, "EX", 3600); // Token มีอายุ 1 ชั่วโมง
              ...
              return cb(null, false, {
                statusCode: 401,
                message:
                  "You need to set a password. An email has been sent to you.",
              });
            }
