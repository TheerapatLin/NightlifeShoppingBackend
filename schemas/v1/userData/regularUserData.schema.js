// schema/v1/regularUserData.schema.js
...
const regularUserDataSchema = new mongoose.Schema(
  {
    ...
    imageUrl: { type: [{ type: String }], default: [] },
    ...
    numPost: { type: Number, default: 0, min: 0 },
    ...
    numUnread: { type: Number, default: 0, min: 0 },
    numCoin: { type: Number, default: 0 },
  },
  { timestamps: true }
);

// ✅ indexes ที่ใช้บ่อย และไม่หนัก
regularUserDataSchema.index({ businessId: 1 });
regularUserDataSchema.index({ gender: 1 });
regularUserDataSchema.index({ nationality: 1 });
// ใช้สำหรับจัดอันดับ/ดึง top creators
regularUserDataSchema.index({ numFollower: -1 });
